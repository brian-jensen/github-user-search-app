...
const toggleHeaderTheme = () => {
  if (body.classList.contains('dark')) {
    themeToggle.innerHTML = `
     light
     <svg class="moon-icon" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
   ...
   themeToggle.innerHTML = `
    dark
    <svg class="sun-icon" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
...
const handleData = data => {
  ...
  card.innerHTML = `
    <section class="user">
      <img class="user-img" src="${avatar_url}" alt="${login}'s profile image">
      <div class="user-info">
        <h3 class="user-name">${name ? name : login.replace('@', '')}</h3>
        <a href="${html_url}" class="user-account" target="_blank">&#64;${login}</a>
        <p class="user-join">Joined ${new Intl.DateTimeFormat(navigator.language, {month: "short", day: "2-digit", year: "numeric" }).format(date).replace(',','')}</p>
      </div>
    </section>
    <aside class="bio">
    ...
    <section class="stats">
      <div class="stat">
        ...
        <h4 class="stat-number">${public_repos}</h4>
      </div>
      <div class="stat">
        <p class="stat-label">Followers</p>
        <h4 class="stat-number">${followers}</h4>
      </div>
      <div class="stat">
        <p class="stat-label">Following</p>
        <h4 class="stat-number">${following}</h4>
      </div>
    </section>
    <footer class="social">
      <ul>
        <li class="location g1" title="${location}">
        ...
        <li class="g2">
          <a href="https://twitter.com/${twitter_username ? twitter_username : ''}" class="twitter-link" target="_blank">
  ...
  const socials = document.querySelectorAll('.social li p');
